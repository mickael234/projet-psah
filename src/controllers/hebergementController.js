...
class HebergementController {
  ...
  static async getAllHebergements(req, res) {
    try {
      ...
      const hebergements = await HebergementModel.findAll(filters, Number.parseInt(page), Number.parseInt(limit))
  ...
  static async checkAvailability(req, res) {
    try {
      ...
      const isAvailable = await HebergementModel.checkAvailability(hebergementId, dateArrivee, dateDepart)
  ...
  static async searchAvailableHebergements(req, res) {
    try {
      const { dateArrivee, dateDepart, page = 1, limit = 10, ...filters } = req.query
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          status: "ERROR",
          message: "Vous n'avez pas les permissions nécessaires pour créer un hébergement",
        })
      }
      ...
      if (!hebergementData.numero_chambre || !hebergementData.type_chambre || !hebergementData.prix_par_nuit) {
        return res.status(400).json({
          status: "ERROR",
  ...
  static async updateHebergement(req, res) {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          status: "ERROR",
          message: "Vous n'avez pas les permissions nécessaires pour modifier un hébergement",
        })
      }
      ...
      const hebergement = await HebergementModel.update(hebergementId, hebergementData)
  ...
  static async deleteHebergement(req, res) {
    try {
      ...
      if (!HebergementController.verifierPermissions(req, ["SUPER_ADMIN", "ADMIN_GENERAL"])) {
        return res.status(403).json({
          status: "ERROR",
          message: "Vous n'avez pas les permissions nécessaires pour supprimer un hébergement",
        })
      }
  ...
  static async addMedia(req, res) {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          status: "ERROR",
          message: "Vous n'avez pas les permissions nécessaires pour ajouter un média",
        })
      }
      ...
      const media = await HebergementModel.addMedia(hebergementId, mediaData)
  ...
  static async removeMedia(req, res) {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          status: "ERROR",
          message: "Vous n'avez pas les permissions nécessaires pour supprimer un média",
        })
      }
      ...
      const mediaExistant = await HebergementModel.findMediaById(mediaIdInt)
