...
class HebergementModel {
  ...
  static async findAll(filters = {}, page = 1, limit = 10) {
    ...
    const where = {
      ...(filters.type_chambre ? { type_chambre: filters.type_chambre } : {}),
      ...(filters.prix_min ? { prix_par_nuit: { gte: parseFloat(filters.prix_min) } } : {}),
      ...(filters.prix_max ? { prix_par_nuit: { lte: parseFloat(filters.prix_max) } } : {}),
   ...
   */
  static async findAvailable(dateArrivee, dateDepart, filters = {}, page = 1, limit = 10) {
    const skip = (page - 1) * limit;
        ...
        ...(equipements && equipements.length > 0 ? {
          equipements: {
            create: equipements.map(id_equipement => ({
              equipement: {
                connect: { id_equipement: parseInt(id_equipement) }
              }
            }))
          }
        } : {})
  ...
  static async update(id, data) {
    ...
    if (equipements && equipements.length > 0) {
      ...
      await Promise.all(equipements.map(id_equipement => 
  ...
  static async delete(id) {
    ...
    if (futureReservations.length > 0) {
      throw new Error('Impossible de supprimer cet hébergement car il a des réservations futures');
    }
