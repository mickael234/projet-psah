...
describe('Ticket Support Model', () => {
     ...
     */
    beforeEach(() => {
        jest.spyOn(prisma.ticketSupport, 'findUnique').mockImplementation(() =>
            Promise.resolve({})
        );
        jest.spyOn(prisma.ticketSupport, 'findMany').mockImplementation(() =>
            Promise.resolve([])
        );
        jest.spyOn(prisma.ticketSupport, 'create').mockImplementation(() =>
            Promise.resolve({})
        );
        jest.spyOn(prisma.ticketSupport, 'update').mockImplementation(() =>
            Promise.resolve({})
        );
    });
    ...
    describe('findById', () => {
        ...
        it("doit propager l'erreur si prisma.ticketSupport.findUnique échoue", async () => {
            ...
            // Act & Assert
            await expect(TicketSupportModel.findById(ticketId)).rejects.toThrow(
                mockError
            );
            expect(prisma.ticketSupport.findUnique).toHaveBeenCalledTimes(1);
        });
    ...
    describe('assignToSelf', () => {
        it('doit appeler prisma.ticketSupport.update pour assigner un ticket à un personnel', async () => {
            ...
            // Act
            const result = await TicketSupportModel.assignToSelf(
                ticketId,
                personnelId
            );
    ...
    describe('updateStatut', () => {
        it("doit appeler prisma.ticketSupport.update pour changer le statut d'un ticket", async () => {
            ...
            // Act
            const result = await TicketSupportModel.updateStatut(
                ticketId,
                newStatut
            );
    ...
    describe('reassignTo', () => {
        it('doit appeler prisma.ticketSupport.update pour réassigner un ticket à un autre personnel', async () => {
            ...
            // Act
            const result = await TicketSupportModel.reassignTo(
                ticketId,
                newPersonnelId
            );
