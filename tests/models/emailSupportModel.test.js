...
describe('Email Support Model', () => {
     ...
     */
    beforeEach(() => {
        jest.spyOn(prisma.emailSupport, 'create').mockImplementation(() =>
            Promise.resolve({})
        );
        jest.spyOn(prisma.emailSupport, 'findMany').mockImplementation(() =>
            Promise.resolve([])
        );
    });
    ...
    describe('sendEmail', () => {
        ...
        it("doit propager l'erreur si prisma.emailSupport.create échoue", async () => {
            ...
            // Act & Assert
            await expect(
                EmailSupportModel.sendEmail(emailData)
    ...
    describe('findByTicket', () => {
        ...
        it("doit propager l'erreur si prisma.emailSupport.findMany échoue", async () => {
            ...
            // Act & Assert
            await expect(
                EmailSupportModel.findByTicket(ticketId)
            ).rejects.toThrow(mockError);
            expect(prisma.emailSupport.findMany).toHaveBeenCalledTimes(1);
        });
    });
