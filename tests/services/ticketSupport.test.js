...
describe('TicketSupportService', () => {
  ...
  describe('getTicketById', () => {
    it('doit récupérer un ticket existant par son ID', async () => {
      // Arrange
      const ticketId = 123;
      const mockTicket = { id_ticket: ticketId, sujet: 'Problème technique' };
    ...
    it('doit lancer ValidationError si l\'ID du ticket est invalide', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.getTicketById(null)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.getTicketById('abc')).rejects.toThrow(ValidationError);
    ...
    it('doit lancer NotFoundError si le ticket n\'existe pas', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.getTicketById(ticketId)).rejects.toThrow(NotFoundError);
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('getTicketsByClient', () => {
    it('doit récupérer les tickets d\'un client existant', async () => {
      ...
      jest.spyOn(ClientModel, 'getWithRelations').mockReturnValue(mockClient);
      jest.spyOn(TicketSupportModel, 'findByClient').mockResolvedValue(mockTickets);

      // Act
      const result = await TicketSupportService.getTicketsByClient(clientId);
      ...
      expect(TicketSupportModel.findByClient).toHaveBeenCalledWith(clientId);
    ...
    it('doit lancer ValidationError si l\'ID client est invalide', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.getTicketsByClient(null)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.getTicketsByClient('abc')).rejects.toThrow(ValidationError);
    ...
    it('doit lancer NotFoundError si le client n\'existe pas', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.getTicketsByClient(clientId)).rejects.toThrow(NotFoundError);
      expect(ClientModel.getWithRelations).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.findByClient).not.toHaveBeenCalled();
    });
  });
  ...
  describe('getAllTickets', () => {
    it('doit récupérer tous les tickets sans filtre', async () => {
      ...
      jest.spyOn(TicketSupportModel, 'findAll').mockResolvedValue(mockTickets);
  ...
  describe('assignTicket', () => {
    it('doit assigner un ticket à un membre du personnel', async () => {
      ...
      const mockPersonnel = { id_personnel: personnelId, nom: 'Support Test' };
      const mockAssignedTicket = { ...mockTicket, id_personnel: personnelId };

      jest.spyOn(TicketSupportModel, 'findById').mockResolvedValue(mockTicket);
      jest.spyOn(PersonnelModel, 'getWithRelations').mockReturnValue(mockPersonnel);
      jest.spyOn(TicketSupportModel, 'assignToSelf').mockResolvedValue(mockAssignedTicket);

      // Act
      const result = await TicketSupportService.assignTicket(ticketId, personnelId);

      // Assert
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.findById).toHaveBeenCalledWith(ticketId);
      expect(PersonnelModel.getWithRelations).toHaveBeenCalledTimes(1);
      expect(PersonnelModel.getWithRelations).toHaveBeenCalledWith(personnelId);
      expect(TicketSupportModel.assignToSelf).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.assignToSelf).toHaveBeenCalledWith(ticketId, personnelId);
    ...
    it('doit lancer ValidationError si les IDs sont invalides', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.assignTicket(null, 123)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.assignTicket(123, null)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.assignTicket('abc', 123)).rejects.toThrow(ValidationError);
      expect(TicketSupportModel.findById).not.toHaveBeenCalled();
    ...
    it('doit lancer NotFoundError si le ticket n\'existe pas', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.assignTicket(ticketId, personnelId)).rejects.toThrow(NotFoundError);
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(PersonnelModel.getWithRelations).not.toHaveBeenCalled();
      expect(TicketSupportModel.assignToSelf).not.toHaveBeenCalled();
    });
    ...
    it('doit lancer NotFoundError si le personnel n\'existe pas', async () => {
      ...
      jest.spyOn(TicketSupportModel, 'findById').mockResolvedValue(mockTicket);
      jest.spyOn(PersonnelModel, 'getWithRelations').mockReturnValue(null);
      jest.spyOn(TicketSupportModel, 'assignToSelf');
  ...
  describe('updateTicketStatus', () => {
    it('doit mettre à jour le statut d\'un ticket', async () => {
      ...
      const mockTicket = { id_ticket: ticketId, sujet: 'Ticket test', statut: 'en_attente' };
      const mockUpdatedTicket = { ...mockTicket, statut };

      jest.spyOn(TicketSupportModel, 'findById').mockResolvedValue(mockTicket);
      jest.spyOn(TicketSupportModel, 'updateStatut').mockResolvedValue(mockUpdatedTicket);

      // Act
      const result = await TicketSupportService.updateTicketStatus(ticketId, statut);

      // Assert
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.findById).toHaveBeenCalledWith(ticketId);
      expect(TicketSupportModel.updateStatut).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.updateStatut).toHaveBeenCalledWith(ticketId, statut);
    ...
    it('doit lancer ValidationError si le statut est invalide', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.updateTicketStatus(ticketId, statut)).rejects.toThrow(ValidationError);
      expect(TicketSupportModel.findById).not.toHaveBeenCalled();
    ...
    it('doit lancer NotFoundError si le ticket n\'existe pas', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.updateTicketStatus(ticketId, statut)).rejects.toThrow(NotFoundError);
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.updateStatut).not.toHaveBeenCalled();
    });
  });
  ...
  describe('reassignTicket', () => {
    it('doit réassigner un ticket à un autre membre du personnel', async () => {
      ...
      const mockTicket = { id_ticket: ticketId, sujet: 'Ticket test', id_personnel: 456 };
      const mockPersonnel = { id_personnel: newPersonnelId, nom: 'Nouveau Support' };
      const mockReassignedTicket = { ...mockTicket, id_personnel: newPersonnelId };

      jest.spyOn(TicketSupportModel, 'findById').mockResolvedValue(mockTicket);
      jest.spyOn(PersonnelModel, 'getWithRelations').mockReturnValue(mockPersonnel);
      jest.spyOn(TicketSupportModel, 'reassignTo').mockResolvedValue(mockReassignedTicket);

      // Act
      const result = await TicketSupportService.reassignTicket(ticketId, newPersonnelId);

      // Assert
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.findById).toHaveBeenCalledWith(ticketId);
      expect(PersonnelModel.getWithRelations).toHaveBeenCalledTimes(1);
      expect(PersonnelModel.getWithRelations).toHaveBeenCalledWith(newPersonnelId);
      expect(TicketSupportModel.reassignTo).toHaveBeenCalledTimes(1);
      expect(TicketSupportModel.reassignTo).toHaveBeenCalledWith(ticketId, newPersonnelId);
    ...
    it('doit lancer ValidationError si les IDs sont invalides', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.reassignTicket(null, 123)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.reassignTicket(123, null)).rejects.toThrow(ValidationError);
      await expect(TicketSupportService.reassignTicket('abc', 123)).rejects.toThrow(ValidationError);
      expect(TicketSupportModel.findById).not.toHaveBeenCalled();
      expect(PersonnelModel.getWithRelations).not.toHaveBeenCalled();
    ...
    it('doit lancer NotFoundError si le ticket n\'existe pas', async () => {
      ...
      // Act / Assert
      await expect(TicketSupportService.reassignTicket(ticketId, newPersonnelId)).rejects.toThrow(NotFoundError);
      expect(TicketSupportModel.findById).toHaveBeenCalledTimes(1);
      expect(PersonnelModel.getWithRelations).not.toHaveBeenCalled();
      expect(TicketSupportModel.reassignTo).not.toHaveBeenCalled();
    });
